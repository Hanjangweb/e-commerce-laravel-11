...
@section('content')
        ...
        <section class="shop-main container d-flex pt-4 pt-xl-5">
                                        ...
                                        <li class="list-item">
                                            <span class="menu-link py-1">
                                                <input type="checkbox" name="categories" value="{{ $category->id }}"
                                                    class="chk-brand" @if (in_array($category->id, explode(',', $f_categories))) checked @endif>
                                                {{ $category->name }}
                                            </span>
            ...
            "pagination": {
              ...
              "clickable": true
                ...
                <div class="d-flex justify-content-between mb-4 pb-md-2">
                    ...
                    <div
                        ...
                        <select class="shop-acs__select form-select w-auto border-0 py-0 order-1 order-md-0"
                            aria-label="page-size" id="pagesize" name="pagesize" style="margin: 5%">
                        ...
                        <select class="shop-acs__select form-select w-auto border-0 py-0 order-1 order-md-0"
                            ...
                            <option value="4" {{ $order == -4 ? 'selected' : '' }}>price, Hign to Low</option>
                            ...
                            <div class="product-card mb-3 mb-md-4 mb-xxl-5">
                                <div class="pc__img-wrapper">
                                    <div class="swiper-container background-img js-swiper-slider"
                                        ...
                                        <div class="swiper-wrapper">
                                            <div class="swiper-slide">
                                                <a href="{{ route('shops.details', ['product_slug' => $shop->slug]) }}"><img
                                                        loading="lazy"
                                                        src="{{ asset('uploads/products') }}/{{ $shop->image }}"
                                                        width="330" height="400" alt="{{ $shop->name }}"
                                                        class="pc__img"></a>
                                            </div>
                                            <div class="swiper-slide">
                                                @foreach (explode(',', $shop->images) as $gimg)
                                                    <a
                                                        href="{{ route('shops.details', ['product_slug' => $shop->slug]) }}"><img
                                                            loading="lazy"
                                    ...
                                    @if (auth()->check())
                                        @if (Cart::instance('cart')->content()->where('id', $shop->id)->isNotEmpty())
                                            <a href="{{ route('cart.index') }}"
                                                class="pc__atc btn anim_appear-bottom btn position-absolute border-0 text-uppercase fw-medium btn btn-warning mb-5">Go
                                                to Cart</a>
                                        @else
                                            <form name="addtocart-form" method="post" action="{{ route('cart.add') }}">
                                                ...
                                                <input type="hidden" name="price"
                                                    value="{{ $shop->sale_price == '' ? $shop->regular_price : $shop->sale_price }}">
                                                <input type="hidden" name="quantity" value="1">
                                                <!-- Ensure this name matches the controller's expectation -->
                                                <button type="submit"
                                    ...
                                    @else
                                        <a href="{{ route('login') }}"
                                            class="pc__atc btn anim_appear-bottom btn position-absolute border-0 text-uppercase fw-medium btn btn-warning mb-5">Login
                                            to Add to Cart</a>
                                    @endif
                                ...
                                <div class="pc__info position-relative">
                                    <p class="pc__category">{{ $shop->category->name }}</p>
                                    <h6 class="pc__title"><a
                                            href="{{ route('shops.details', ['product_slug' => $shop->slug]) }} ">{{ $shop->name }}</a>
                                    </h6>
                                    <div class="product-card__price d-flex">
                                        <span class="money price">

                                            @if ($shop->sale_price)
                                                <s>${{ $shop->regular_price }}</s> {{ $shop->sale_price }}
                                            @else
                                                ${{ $shop->regular_price }}
                                            @endif
                                        </span>
                                    ...
                                    @if (Cart::instance('wishlist')->content()->where('id', $shop->id)->count() > 0)
                                        <form
                                            action="{{ route('wishlist.item.remove', ['rowId' => Cart::instance('wishlist')->content()->where('id', $shop->id)->first()->rowId]) }}"
                                            method="POST" style="display: inline;">
                                            @csrf
                                            @method('DELETE')
                                            <button type="submit"
                                                class="pc__btn-wl position-absolute top-0 end-0 bg-transparent border-0  "
                                                style="color: orange;" title="Add To Wishlist">
                ...
                <div class="flex items-center justify-between flex-wrap gap10 wgp-paginaton">
    ...
    <form action="{{ route('shops.index') }}" id="frmfilter" method="GET">
        @csrf
        <input type="hidden" name="page" value="{{ $shops->currentPage() }}">
    ...
    <script>
        $(function() {
            $("#pagesize").on("change", function() {
                // console.log("Selected size:", $("#pagesize option:selected").val());
            ...
            $("input[name='brands']").on('change', function() {
                ...
                $("#frmfilter").submit(); // Corrected: Added '#' to target the form by ID
            });
            ...
            $("[name='price_range']").on("change", function() {
                var values = $(this).val().split(','); // Split the value into an array
                var min = values[0]; // Get the first value (min price)
                var max = values[1]; // Get the second value (max price)
                $("#hdnminPrice").val(min); // Set the hidden min price input value
                $("#hdnmaxPrice").val(max); // Set the hidden max price input value
